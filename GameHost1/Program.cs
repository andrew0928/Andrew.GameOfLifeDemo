...
namespace GameHost1
{
    public class Program
    {
        ...
        private static int GetAroundAliveCount(bool[,] area)
        {
            ...
            var idx = 0;
            var count = 0;
            foreach (bool status in area)
            {
                if (idx==middleIndex) continue;
                if (status) count += 1;
                idx += 1;
            }
        ...
        {
            int aroundAliveCount = GetAroundAliveCount(area);

            return IsAlive(area)
                ? KeepAliveCount.Contains(aroundAliveCount)
                : RebornCount == aroundAliveCount;
        }
